...
registerOption('calibEn',     False,    'Use EGM smearer to calibrate photon and electron energy')  # FIXME TODO: check with Priya if we use this in data analysis (we must use same setting here)
...
registerOption('era',         'UL2016',   'Data-taking era: 2016, 2017, 2018, UL2017 or UL2018', optionType=VarParsing.varType.string)
...
if varOptions.era not in ['2016', '2017', '2018', 'UL2016preVFP', 'UL2016postVFP', 'UL2016','UL2017', 'UL2018']: 
...
options['LOWPTELECTRON_CUTS']   = "ecalEnergy*sin(superClusterPosition.theta)>1.0 &&  (abs(-log(tan(superClusterPosition.theta/2)))<2.5)" # we need to go to 3 or 1 in Et, eta cut can stay same
...
if varOptions.GT == "auto":
  if options['isMC']:
    ...
    # added by me
    if options['era'] == 'UL2016':  options['GLOBALTAG'] = '106X_mcRun2_asymptotic_v17'
  else:
    ...
    # added by me
    if options['era'] == 'UL2016':  options['GLOBALTAG'] = '106X_dataRun2_v27'
else:
...
if options['DoEleID'] or options['DoTrigger'] : process.cand_sequence += process.lowptele_sequence #
...
if options['DoTrigger'] : process.tnpPairs_sequence *= process.tnpPairingLowPtEleHLT #
...
if options['DoEleID']   : process.tnpPairs_sequence *= process.tnpPairingLowPtEleIDs #
...
if (options['DoTrigger']): process.tree_sequence *= process.tnpLowPtEleTrig #
if (options['DoRECO'])   : process.tree_sequence *= process.tnpEleReco
if (options['DoEleID'])  : process.tree_sequence *= process.tnpEleIDs
if (options['DoEleID'])  : process.tree_sequence *= process.tnpLowPtEleIDs #
